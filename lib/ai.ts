...
export async function generateDeploymentSummary(
  ...
  if (!genAI) {
    ...
    return {
      summary: deployment.status === 'succeeded' 
...
Duration: ${deployment.duration ? `${Math.round(deployment.duration / 1000)}s` : 'unknown'}
${deployment.url ? `URL: ${deployment.url}` : ''}
${deployment.errorMessage ? `Error: ${deployment.errorMessage}` : ''}
${deployment.buildLogs ? `Build Logs (last 3000 chars):\n\`\`\`\n${deployment.buildLogs.slice(-3000)}\n\`\`\`` : ''}
...
${deployment.status === 'failed' ? 'Focus on identifying the root cause and providing specific fix recommendations.' : 'Keep it brief and highlight what was deployed.'}
Be concise and actionable. Limit each array to 3 items max.`
  ...
  try {
    ...
    const jsonMatch = response.match(/\{[\s\S]*\}/)
    if (!jsonMatch) {
      console.error('❌ No JSON found in deployment response:', response.substring(0, 200))
      throw new Error('Failed to parse AI response - no JSON found')
    }
    ...
    return {
      ...
      potentialIssues: Array.isArray(parsed.potentialIssues) ? parsed.potentialIssues.slice(0, 3) : [],
      recommendations: Array.isArray(parsed.recommendations) ? parsed.recommendations.slice(0, 3) : [],
      impactLevel: ['low', 'medium', 'high'].includes(parsed.impactLevel) ? parsed.impactLevel : 'medium'
    }
  } catch (error) {
    const errorMsg = error instanceof Error ? error.message : String(error)
    ...
    return {
      summary: deployment.status === 'succeeded' 
        ? `Deployment to ${deployment.environment} completed successfully.`
        : `Deployment to ${deployment.environment} failed: ${deployment.errorMessage || 'Unknown error'}`,
      highlights: deployment.commitMessage ? [deployment.commitMessage] : [],
      potentialIssues: deployment.errorMessage ? [deployment.errorMessage] : [],
      recommendations: deployment.status === 'failed' ? ['Check build logs for details', 'Verify environment variables'] : [],
      impactLevel: deployment.environment === 'production' ? 'high' : 'medium'
    }
  }
}
